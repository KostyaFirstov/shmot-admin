...
import { fetchRequests, selectRequest } from '../redux/slices/requests'
import TableRequests from '../components/Tables/TableRequests'

const Requests = () => {
	const { requests, status } = useSelector(selectRequest)
	const appDispatch = useAppDispatch()

	React.useEffect(() => {
		appDispatch(fetchRequests())
	}, [])
	...
	return (
		<ContentLayout title='Все запросы'>
			<StatisticBlock cards={requestsCards} status={status} />
			<TableRequests
				headers={['Текст запроса', 'Видимость', 'Кол. ведённых раз']}
				requests={requests}
				status={status}
			/>
		</ContentLayout>
