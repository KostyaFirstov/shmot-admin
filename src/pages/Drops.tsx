...
import { fetchDrops, selectDrops } from '../redux/slices/drops'
import TableDrops from '../components/Tables/TableDrops'
import StatisticBlock from '../components/StatisticBlock'

const Drops = () => {
	const appDispatch = useAppDispatch()
	const { drops, status } = useSelector(selectDrops)
	...
	return (
		<ContentLayout title='Предстоящие дропы'>
			<StatisticBlock cards={dropsCards} status={status} />
			<TableDrops
				headers={['Название', 'Просмотров', 'Дата дропа', 'Дата создания']}
				drops={drops}
				status={status}
			/>
		</ContentLayout>
