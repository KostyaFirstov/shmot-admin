...
const Main = () => {
	...
	const [statisticCards, setStatisticCards] = React.useState([
		{
			icon: (
				<svg
					width='65'
		...
		},
		{
			...
			name: 'Пользователи',
			param: percUser?.param,
			error: percUser?.error ? Boolean(percUser.error) : true
		},
		{
			icon: (
				<svg
	...
	React.useEffect(() => {
		const getStats = async () => {
			try {
				const { data } = await axios.get<UserStatsReq[]>('/api/users/stats')
				data.map(item => {
					setUserStats(prev => [
						...prev,
				...
				setPercUser({
					error:
						(+data[data.length - 2].total * 100) / +data[data.length - 1].total,
						...
						<div className='statistic__card statistic__card-members'>
							...
							<div className='card-members__list'>
								{users.map((user, index) => {
									return (
										<Link key={index} to={'/'} className='card-members__item'>
