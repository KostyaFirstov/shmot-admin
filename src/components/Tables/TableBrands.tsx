...
const Table: React.FC<ITableProps> = ({ headers, brands, status }) => {
	...
	const handleRemove = (id: number) => {
		if (window.confirm('Вы точно хотите удалить бренд?')) {
			...
			appDispatch(fetchRemoveBrand(id))
		}
	}

	return (
		<div className='table'>
			{status === 'loaded' && brands.length === 0 ? (
				<NotFound title='Пусто :(' desc='Ни одного бренда не найдено' />
			) : status === 'error' ? (
				<NotFound
					title='Ошибка...'
					desc='Не удалось загрузить бренды, попробуйте позже!'
				/>
			) : (
				<>
