...
const Table: React.FC<ITableProps> = ({ headers, requests, status }) => {
	const dispatch = useDispatch()
	const appDispatch = useAppDispatch()

	const handleRemove = (id: number) => {
		if (window.confirm('Вы точно хотите удалить запрос?')) {
			dispatch(removeRequest(id))
			appDispatch(fetchRemoveRequests(id))
		}
	}
	...
	return (
		<div className='table'>
			{status === 'loaded' && requests.length === 0 ? (
				<NotFound title='Пусто :(' desc='Ни одного запроса не найдено' />
			) : status === 'error' ? (
				<NotFound
					title='Ошибка...'
					desc='Не удалось загрузить запросы, попробуйте позже!'
				/>
			) : (
				<>
						...
						{status === 'loading'
							...
							: requests.map((item, index) => (
									<div key={index} className='table-main__item-outer'>
										<Link
											to={`/admin/request-add/${item._id}/edit`}
										...
										>
											...
											<div className='table-main__inner'>
												<span>{item.isApproved ? 'Видим' : 'Не видим'}</span>
											</div>
											<div className='table-main__inner'>
