...
import { fetchRemoveReviews, removeReview } from '../../redux/slices/reviews'
...
const Table: React.FC<ITableProps> = ({ headers, reviews, status }) => {
	...
	const handleRemove = (id: number) => {
		if (window.confirm('Вы точно хотите удалить бренд?')) {
			...
			appDispatch(fetchRemoveReviews(id))
		}
	}
	...
	return (
		<div className='table'>
			{status === 'loaded' && reviews.length === 0 ? (
				<NotFound title='Пусто :(' desc='Ни одного обзора не найдено' />
			) : status === 'error' ? (
				<NotFound
					title='Ошибка...'
					desc='Не удалось загрузить обзоры, попробуйте позже!'
				/>
			) : (
				<>
						...
						{status === 'loading'
							...
							: reviews.map((item, index) => (
									<div key={index} className='table-main__item-outer'>
										<Link
											to={`/admin/review-add/${item.title}/edit`}
